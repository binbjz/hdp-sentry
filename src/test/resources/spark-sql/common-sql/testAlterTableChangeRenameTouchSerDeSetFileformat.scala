...
val test_sql="ALTER TABLE testdb_spark.tbl4fileformat PARTITION (year=2012, month=1, day=1) SET FILEFORMAT SEQUENCEFILE";
spark.sql(test_sql).collect().foreach(println);
val test_sql="DESCRIBE EXTENDED testdb_spark.tbl4fileformat PARTITION (year=2012, month=1, day=1)";
spark.sql(test_sql).collect().foreach(println);
val test_sql="ALTER TABLE testdb_spark.tbl4fileformat SET FILEFORMAT SEQUENCEFILE";
spark.sql(test_sql).collect().foreach(println);
val test_sql="DESCRIBE EXTENDED testdb_spark.tbl4fileformat";
spark.sql(test_sql).collect().foreach(println);

val test_sql="ALTER TABLE testdb_spark.tbl4fileformat_external PARTITION (year=2012, month=1, day=1) SET FILEFORMAT SEQUENCEFILE";
spark.sql(test_sql).collect().foreach(println);
val test_sql="DESCRIBE EXTENDED testdb_spark.tbl4fileformat_external PARTITION (year=2012, month=1, day=1)";
spark.sql(test_sql).collect().foreach(println);
val test_sql="ALTER TABLE testdb_spark.tbl4fileformat_external SET FILEFORMAT SEQUENCEFILE";
spark.sql(test_sql).collect().foreach(println);
val test_sql="DESCRIBE EXTENDED testdb_spark.tbl4fileformat_external";
spark.sql(test_sql).collect().foreach(println);
