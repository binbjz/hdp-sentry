...
var data_source_sql="SELECT * FROM mart_waimai.test_insert_overwrite_dir se WHERE se.country = 'US' and se.state = 'CA'";
...
val parquetPath="/user/hive/warehouse/mart_waimai.db/spark_ca_employees_parquet"
...
val df_parquet_query = spark.sql("SELECT * FROM parquet.`/user/hive/warehouse/mart_waimai.db/spark_ca_employees_parquet`")
df_parquet_query.collect.foreach(println);

/* save df data to table */
df_parquet.write.mode(SaveMode.Overwrite).saveAsTable("mart_waimai.spark_df_parquet")
df_parquet.write.insertInto("mart_waimai.spark_df_parquet")

df_parquet_query.write.mode(SaveMode.Overwrite).saveAsTable("mart_waimai.spark_df_parquet_query")
df_parquet_query.write.insertInto("mart_waimai.spark_df_parquet_query")
...
val df_json_query = spark.sql("SELECT * FROM json.`/user/hive/warehouse/mart_waimai.db/spark_ca_employees_json`");
df_json_query.collect.foreach(println);

/* save df data to table */
df_json.write.mode(SaveMode.Overwrite).saveAsTable("mart_waimai.spark_df_json");
df_json.write.insertInto("mart_waimai.spark_df_json");

df_json_query.write.mode(SaveMode.Overwrite).saveAsTable("mart_waimai.spark_df_json_query");
df_json_query.write.insertInto("mart_waimai.spark_df_json_query");
...
/* query data and write to HDFS dir */
val orcPath="/user/hive/warehouse/mart_waimai.db/spark_ca_employees_orc"
...
val df_orc_query = spark.sql("SELECT * FROM orc.`/user/hive/warehouse/mart_waimai.db/spark_ca_employees_orc`")
df_orc_query.collect.foreach(println);

/* save df data to table */
df_orc.write.mode(SaveMode.Overwrite).saveAsTable("mart_waimai.spark_df_orc")
df_orc.write.insertInto("mart_waimai.spark_df_orc")

df_orc_query.write.mode(SaveMode.Overwrite).saveAsTable("mart_waimai.spark_df_orc_query")
df_orc_query.write.insertInto("mart_waimai.spark_df_orc_query")
