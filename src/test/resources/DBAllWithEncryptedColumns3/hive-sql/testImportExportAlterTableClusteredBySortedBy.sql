...
dfs -cat /user/hive/warehouse/encrypt_db4data.db/src_tgt_employees/country=US/state=CA/california-employees.csv;

-- 导出分区并且导入到分区表分区
EXPORT TABLE encrypt_db4data.src_tgt_employees PARTITION (country = 'US', state = 'CA') TO '/tmp/employee';
...
ALTER TABLE encrypt_db4data.src_tgt_employees TOUCH PARTITION (country = 'US', state = 'CA') ;
...
dfs -rm /user/hive/warehouse/encrypt_db4data.db/src_tgt_employees/country=US/state=CA/california-employees.csv;
