...
ALTER TABLE db4data.src_employees ADD PARTITION (country = 'US', state = 'CA');
...
SELECT * FROM db4data.src_employees;

FROM db4data.src_employees se
...
INSERT OVERWRITE TABLE db4data.employees PARTITION (country = 'US', state = 'CA')
SELECT name, salary, subordinates, deductions, address FROM db4data.src_employees se WHERE se.country = 'US' AND se.state = 'CA';
SELECT * FROM db4data.employees;
...
SELECT se.name, se.salary, se.subordinates, se.deductions, se.address, se.state FROM db4data.src_employees se WHERE se.country = 'US';
SELECT * FROM db4data.employees;

INSERT OVERWRITE LOCAL DIRECTORY '/tmp/ca_employees'
SELECT * FROM db4data.src_employees se WHERE se.country = 'US' and se.state = 'CA';
!ls -l /tmp/ca_employees;
!rm -r /tmp/ca_employees;

FROM (
      SELECT emp.name, emp.salary FROM db4data.src_employees emp WHERE emp.salary < 6000
      UNION ALL
      SELECT emp.name, emp.salary FROM db4data.src_employees emp WHERE emp.salary > 7000
) unioninput
INSERT OVERWRITE DIRECTORY '/tmp/union.out' SELECT unioninput.*;
dfs -cat /tmp/union.out/* ;
dfs -rm -r /tmp/union.out ;

ANALYZE TABLE db4data.src_employees COMPUTE STATISTICS FOR columns name, salary;
ANALYZE TABLE db4data.src_employees PARTITION (country = 'US', state = 'CA') COMPUTE STATISTICS;
ANALYZE TABLE db4data.src_employees PARTITION (country = 'US', state)  COMPUTE STATISTICS;
