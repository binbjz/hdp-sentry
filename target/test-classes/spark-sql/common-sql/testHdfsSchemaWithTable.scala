...
val hdfsPath="/user/hive/warehouse/testdb.db/test_schema"
...
df.printSchema()
df.write.mode(SaveMode.Overwrite).saveAsTable("testdb.spark_merge_schema_true")
val test_sql="SELECT * FROM testdb.spark_merge_schema_true ORDER BY 1";
spark.sql(test_sql).collect().foreach(println);


val df = spark.read.option("mergeSchema", "false").parquet(hdfsPath)
df.printSchema()
df.write.partitionBy("key").mode(SaveMode.Overwrite).saveAsTable("testdb.spark_merge_schema_false")
val test_sql="SELECT * FROM testdb.spark_merge_schema_false ORDER BY 1";
spark.sql(test_sql).collect().foreach(println);
...
/* cleanup HDFS dir */
val csvPath="/user/hive/warehouse/testdb.db/datafile.csv"
