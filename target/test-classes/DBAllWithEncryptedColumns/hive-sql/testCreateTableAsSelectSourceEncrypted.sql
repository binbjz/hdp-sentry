...
FROM (SELECT a.a_user_id as c_user_id
      ...
      FROM (SELECT encrypt_user_id as a_user_id
                  ,encrypt_pageview_id as a_pageview_id
                  ,encrypt_timestamp as a_timestamp
           FROM encrypt_db4data.session_src_encrypted ) a
       JOIN (SELECT encrypt_user_id as b_user_id
                   ,encrypt_timestamp as b_timestamp
